...
func TestDKVDiscoveryService(t *testing.T) {
    dkvSvc, grpcSrvr = setupDiscoveryServer()
    defer dkvSvc.Close()
...
func setupDiscoveryServer() (master.DKVService, *grpc.Server) {
    dkvSvc, grpcSrvr = master.ServeStandaloneDKV(&serverpb.RegionInfo{}, dbFolder)

    discoverServiceConf := NewDiscoverConfig(5, 2)
    discoveryService := NewDiscoveryService(dkvSvc, zap.NewNop(), discoverServiceConf)
